...
function request(path: string, method = 'GET', origin = 'https://memorioso.test') {
  return new NextRequest(`https://memorioso.test/api/libro/browser/${path}`, { method,
    ...
    ...(method === 'GET' ? {} : { body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'identity_verify', arguments: {} } }) }) })
}
...
describe('Memorioso browser bridge', () => {
  ...
  it('forwards only Libro cookies and scopes returned session cookies to the browser bridge', async () => {
    ...
    expect(url.toString()).toBe('https://libro.test/mcp')
  ...
  it.each([
    ...
    const response = await POST(request('mcp', 'POST'), context('mcp'))
  ...
  it.each(['oauth/token', 'api/v1/me', 'api/internal/events/deliver', '../oauth/token', 'api/v1/signing/cap/relay'])('does not proxy arbitrary paths: %s', async (path) => {
...
describe('MCP browser allowlist', () => {
  beforeEach(() => {
    vi.stubEnv('LIBRO_SERVICE_URL', 'https://libro.test')
    vi.stubEnv('NEXT_PUBLIC_APP_URL', 'https://memorioso.test')
    fetcher.mockReset()
  ...
  it.each(['update_profile', 'create_human_publication', 'list_publications', 'publication_counts', 'revoke_agent', 'deliver_events', 'oauth_token', 'oauth_register', 'oauth_revoke'])('does not expose %s to browser cookies', async (name) => {
    const req = new NextRequest('https://memorioso.test/api/libro/browser/mcp', {
      method: 'POST', headers: { Origin: 'https://memorioso.test', 'Content-Type': 'application/json' },
      body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name, arguments: {} } }),
    })
    expect((await POST(req, context('mcp'))).status).toBe(403)
    expect(fetcher).not.toHaveBeenCalled()
  })

  it('forwards SSE tool results and empty notification acknowledgements', async () => {
    const wire = 'event: message\ndata: {"jsonrpc":"2.0","id":1,"result":{"content":[]}}\n\n'
    fetcher.mockResolvedValueOnce(new Response(wire, { headers: { 'Content-Type': 'text/event-stream' } }))
      .mockResolvedValueOnce(new Response(null, { status: 202 }))
    const response = await POST(request('mcp', 'POST'), context('mcp'))
    expect(response.headers.get('Content-Type')).toBe('text/event-stream')
    expect(await response.text()).toBe(wire)
    const notification = await POST(request('mcp', 'POST'), context('mcp'))
    expect(notification.status).toBe(202)
    expect(await notification.text()).toBe('')
  })

  it('rejects cross-origin MCP writes before contacting Libro', async () => {
    expect((await POST(request('mcp', 'POST', 'https://attacker.test'), context('mcp'))).status).toBe(403)
    expect(fetcher).not.toHaveBeenCalled()
  })
})
