...
export function getWorldIdServerConfig(): WorldIdServerConfig {
  const environment =
    process.env.NEXT_PUBLIC_WORLD_ID_ENVIRONMENT === "staging"
      ? "staging"
      : "production";
...
export async function verifyWorldIdProof(
  ...
  const response = await fetch(
    `https://developer.worldcoin.org/api/v4/verify/${rpId}`,
    {
      method: "POST",
  ...
  try {
    ...
    body = {
      success: false,
      message: "World verifier returned a non-JSON response",
    };
  }
