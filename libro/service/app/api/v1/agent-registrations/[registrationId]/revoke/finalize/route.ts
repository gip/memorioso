...
export async function PUT(request: Request, context: { params: Promise<{ registrationId: string }> }) {
  try {
    ...
    if (typeof body.transactionHash !== 'string') return Response.json({ error: 'Transaction hash required' }, { status: 400 })
    return Response.json(await revokeAgent(principal, registrationId, body.transactionHash), { headers: { 'Cache-Control': 'no-store' } })
