...
import { execute } from './identity_verify'
import { execute as hint } from './identity_hint'
...
vi.mock('./context', () => ({ setBrowserCookie: mocks.set }))
...
it.each(['signup', 'login'] as const)('remembers a verified %s for a year and proposes it after logout without authenticating', async (purpose) => {
  const sessionId = `session_${'ab'.repeat(64)}`
  await execute({ purpose, handle: 'alice', payload: { session_id: sessionId } })
  ...
  expect(await hint({})).toEqual({ continueAs: 'alice', authenticated: false })
...
it('does not remember a name when World verification fails', async () => {
  ...
  await expect(execute({ purpose: 'login', handle: 'alice', payload: {} })).rejects.toThrow('Invalid proof')
