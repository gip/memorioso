...
import { browserUrl, mcpStateSecret, oauthIssuer, WRITE_GRANT_MAX_AGE_SECONDS } from '@/lib/config'
...
export async function authorizationContext(query: string) {
  const url = new URL(browserUrl('/authorize'))
  url.search = query
  ...
  setBrowserCookie(CONSENT_COOKIE, nonce, { ...identitySessionCookieOptions, maxAge: 600 })
  return { clientId: input.clientId, displayName: String(input.client.display_name || input.clientId),
    resource: input.resource, scope: input.scope, consent: token }
}

export async function authorize(consentToken: string, decision?: 'deny') {
  const consent = verifyState<Consent>(consentToken, mcpStateSecret())
  ...
  setBrowserCookie(CONSENT_COOKIE, '', { ...identitySessionCookieOptions, maxAge: 0 })
  const destination = new URL(input.redirectUri)
  if (decision === 'deny') destination.searchParams.set('error', 'access_denied')
  else destination.searchParams.set('code', await issueAuthorizationCode({ ...input, identityId }))
  destination.searchParams.set('iss', oauthIssuer())
